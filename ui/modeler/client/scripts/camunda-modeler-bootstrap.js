...
window.__define('camunda-modeler-bootstrap', [
  './scripts/camunda-modeler-ui'
], function() {
  'use strict';

  var camundaModelerUi = window.CamundaModelerUi;
  ...
  camundaModelerUi.exposePackages(requirePackages);
  ...
  requirejs(['globalize'], function(globalize) {
    ...
    requirejs(dependencies, function(angular) {
      // we now loaded the modeler and the plugins, great
      // before we start initializing the modeler though (and leave the requirejs context),
      // lets see if we should load some custom scripts first

      if (typeof window.camModelerConf !== 'undefined' && window.camModelerConf.customScripts) {
        var custom = window.camModelerConf.customScripts || {};
        ...
        requirejs(custom.deps || [], function() {
          ...
          angular.module('cam.modeler.custom', custom.ngDeps);
          ...
          // initialize modeler
          camundaModelerUi(pluginDependencies);
        });
      }
      else {
        // for consistency, also create a empty module
        angular.module('cam.modeler.custom', []);
...
requirejs(['camunda-modeler-bootstrap'], function() {});
